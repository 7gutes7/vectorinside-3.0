...
export default function Topography({
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      const ma = morphAmount;
      const sp = speed;
      const msp = morphSpeed;
      for (let g = 0; g < 4; g++) {
  ...
  return <div ref={containerRef} className={`topography-container ${className}`.trim()} style={{ width: '100%', height: '100%', position: 'relative', ...style }} />;
}
